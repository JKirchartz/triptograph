            ...
            function(err, resp, body) {
                if (!err && resp.statusCode == 200) {
                    ...
                    data = data.query.allimages.filter(function(i) {
                        return ["jpg", "jpeg", "png", "bmp", "tiff", "gif"].includes(i.url.split('.').pop());
                    });
                    if (resultType == "json") {
                    ...
                    var images = [];
                    for (var i = 0; i < 5; i++) {
                        images.push(data.splice(Math.floor(Math.random() * data.length), 1)[0]);
                    }
                    var output;
...
app.get("/hits", function (request, res) {
  get(glitchApiUrl + "/projects/" + request.query.project_name + "?authorization=" + glitchAuthToken,
